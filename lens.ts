...
function root<T>(): Path<T, T> {
    return [];
}

type Key<T> = keyof NonNullable<T> | Lens<NonNullable<T>, any>;

type Type<Root, T> = T extends Lens<NonNullable<Root>, infer R>
    ? R
    : T extends keyof NonNullable<Root>
    ? NonNullable<Root>[T]
    : never;
...
function path<
    Root,
    A extends Key<Root>,
    AT extends Type<Root, A>,
    B extends Key<AT>,
    BT extends Type<AT, B>,
    C extends Key<BT>,
    CT extends Type<BT, C>
>(r: Path<Root, Root>, a: A, b: B, c: C): Return<Root | AT | BT, CT> {}
